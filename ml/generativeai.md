...
还有一种**自注意力机制**，帮助文本挖掘每个单词和句子中其他单词的关联。在性能上，超过普通的序列到序列模型。

现在回到**Transformer**：它的每个编码器有两个主要部分，自注意力机制和前馈神经网络。每个解码器由三个部分，自注意力机制，针对编码器的注意力机制，和前馈神经网络。

（这个前馈神经网络是什么玩意儿，我也不知道，然后我就查了一下，其实就是正向传播的普通的神经网络层而已，输入，隐藏到输出。以后看到这个词别被吓到。）

注意力方面，Transformer使用的是多头注意力机制，这比一般的注意力和自注意力更进一步，你一个人看不全的重点，计算机可以多个一起看。通过多个头同时进行分析，然后得出加权组合，得到可能性最大的关联性词语，然后进行结果输出。

同时Transformer还具有**位置编码**机制，要知道词语的前后顺序不同有时候，意思也很大不同，所以就需要位置编码让分析更准确。如何加入位置编码，就是在每个单词的编码信息中，加入一个表示它在序列中的位置的向量。

以上的Transformer的超能力，奠定了GPT模型的根基。
